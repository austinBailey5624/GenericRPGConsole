...
{    
	...
	public void displayMenu(Party party)
	{
		...
		while(!exit)
		{
			...
			in=JOptionPane.showInputDialog("Welcome to our shop! feel free to buy stuff! We'd like that!\n1) Buy\n2) Sell\n3) Leave");
			if(verifyInt(in))
			{
			...
			{
			    JOptionPane.showMessageDialog(null, "Thank you for your custom, please come again!");
				System.out.println("Thank you for your custom, please come again!");
				return;
			}
			else
			{
			    JOptionPane.showMessageDialog(null, "Sorry we didn't understand your input, please enter a 1 a 2 or a 3");
	...
	{	    
		...
		while(!exit)
		{
		    ...
		    text.append("You currently have " + party.getGold() +" Gold avaliable to buy stuff with\n");
			System.out.println("You currently have " + party.getGold() +" Gold avaliable to buy stuff with");
			
			text.append("Please select what you would like to buy\n");
			...
			for(int i=0;i<Item.getAllItems().length;i++)
			{
				if(m_inventory[i]>0)
				{
				    text.append(choice + ") " + itemSet[i].getName());
					...
					text.append(" costs: " + itemSet[i].getValue());
			...
			{
			    JOptionPane.showMessageDialog(null, "Sorry we cant understand you input, please enter one of the numbers provided");
			...
			else if(indexRepresentedByChoice[select]!=-1)
			{
				...
				{
				    JOptionPane.showMessageDialog(null, "Sorry, we don't have any of those in stock");
				...
				else if(m_inventory[indexRepresentedByChoice[select]]>1)
				{
					...
					in=JOptionPane.showInputDialog("How many " + itemSet[indexRepresentedByChoice[select]].getName() +" would you like to buy?");
					if(verifyInt(in))
					{
					...
					}
					{
						if(quantity<0)
						{
						    JOptionPane.showMessageDialog(null, "Sorry, we cant give you a negative number, you can sell to us though!");
						...
						{
						    JOptionPane.showMessageDialog(null, "Sorry, we don't have that many!");
						...
						{
						    JOptionPane.showMessageDialog(null, "Sorry we don't undersand your input, please try again!");
	...
	public void purchase(int index,Party character)
	{
		...
		if(character.canBuyItem(itemSet[index].getValue()))//puchase is successful
		{
			...
			{
			    JOptionPane.showMessageDialog(null, "Puchase successful! You have bought one " + itemSet[index].getName());
				System.out.println("Puchase successful! You have bought one " + itemSet[index].getName());
				this.m_inventory[index]--;
			}
			else
			{
			    JOptionPane.showMessageDialog(null, "ERROR purchase not successful, please try again");
	...
	public void purchaseMultiple(int index, int quantity, Party character)
	{
		...
		if(character.canBuyItems(index, quantity))//purchase is successful
		{
			...
			{
			    JOptionPane.showMessageDialog(null, "Purchase successful! You have bought" + quantity + " " +itemSet[index].getName());
	...
	public void displaySellMenu(Party party)
	{
		...
		while(!exit)
		{
			...
			{
			    JOptionPane.showMessageDialog(null, "Sorry, you don't have anything to sell...");
				System.out.println("Sorry, you don't have anything to sell...");
				return;
			}
			text.append("You currently have: " + party.getGold() + " gold\n");
			System.out.println("You currently have: " + party.getGold() + " gold");
			text.append("What would you like to sell?\n");
			...
			for(int i=0;i<party.getInventory().length; i++)//displays possible sell options
			{
				...
				{
				    text.append(choice + ") sell " + itemSet[i].getName() + " for " + ((int)itemSet[i].getValue()*.8) + "\n");
			...
			System.out.println((choice) + ") Leave");
			in=JOptionPane.showInputDialog((choice) + ") Leave");
			if(verifyInt(in))
			{
				select=Integer.parseInt(in);
			}
	...
	public void sell(int index, Party character)
	{
		...
		JOptionPane.showMessageDialog(null, "Successfully sold one " + itemSet[index].getName() + " for " + (int)(itemSet[index].getValue()*.8) + " gold.");
	...
	public void sellMultiple(int index, Party character)
	{
		...
		while(!exit)
		{
			...
			in=JOptionPane.showInputDialog("How many " + itemSet[index].getName() + "Would you like to sell?\n(if you would like to go back, sell 0)");
			if(verifyInt(in))
			{
				select=Integer.parseInt(in);
			}
			...
			{
			    JOptionPane.showMessageDialog(null, "Sorry you have to sell at least one item");
				System.out.println("Sorry you have to sell at least one item");
			}
			else if(select>tempInventory[index])
			{
			    JOptionPane.showMessageDialog(null, "Sorry you dont have that many");
			...
			else if(select<=tempInventory[index])
			{
				...
				JOptionPane.showMessageDialog(null, "Successfully sold " + select + " " + itemSet[index].getName() + " for " + ((int)(itemSet[index].getValue()*.8*select)) + " gold");
				System.out.println("Successfully sold " + select + " " + itemSet[index].getName() + " for " + ((int)(itemSet[index].getValue()*.8*select)) + " gold");
			}
			else
			{
			    JOptionPane.showMessageDialog(null, "Sorry, we don't understand your input");
