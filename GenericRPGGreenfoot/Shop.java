...
public class Shop
{
    ...
    public void displayMenu(Party party)
    {
        ...
        while(!exit)
        {
            ...
            in = Greenfoot.ask("Welcome to our shop! feel free to buy stuff! We'd like that!\n1) Buy\n2) Sell\n3) Leave");
            if(verifyInt(in))
            {
        ...
        while(!exit)
        {
            ...
            else if(indexRepresentedByChoice[select]!=-1)
            {
                ...
                else if(m_inventory[indexRepresentedByChoice[select]]>1)
                {
                    ...
                    //in=myScanner.next();
                    in = Greenfoot.ask("How many " + itemSet[indexRepresentedByChoice[select]].getName() +" would you like to buy?");
                    if(verifyInt(in))
                    {
    ...
    public void sellMultiple(int index, Party character)
    {
        ...
        while(!exit)
        {
            ...
            //in=myScanner.next();
            in = Greenfoot.ask("How many " + itemSet[index].getName() + "Would you like to sell?\n(if you would like to go back, sell 0)");
            if(verifyInt(in))
            {
                select=Integer.parseInt(in);
            }
